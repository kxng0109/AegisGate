 ...
 * {@link DefaultRedisScript} bean. Spring Data Redis executes the script via
 * {@code EVALSHA} and transparently falls back to {@code EVAL} when the script
 * is not yet cached on the server (the NOSCRIPT path is handled inside Spring,
 * not by the engine).</p>
 */
@Configuration
public class RateLimitScriptConfig {
	 ...
	 * list arrive as {@link String} values when the connection uses the
	 * {@code StringRedisTemplate} serializer or as {@link Long} values with a
	 * native serializer; the engine tolerates both.</p>
	 *
