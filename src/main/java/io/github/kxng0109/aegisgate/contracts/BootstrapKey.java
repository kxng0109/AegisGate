...
/**
 * A virtual API key seeded at startup from configuration (used until the
 * deferred admin/JWT path provides full CRUD). Bound from
 * {@code gateway.bootstrap-keys}.
 *
 * @param ownerId          owner/tenant id
 * @param name             label
 * @param plaintextKey     raw key including the {@code gw-} prefix (must be a
 *                         real secret in practice; here it is config-supplied)
